 ...
 * @since       11.1
 */
class RokCommon_Form_Field_Rules extends RokCommon_Form_AbstractField
{
	/**
	 * The form field type.
	 *
	...
	public function getInput()
	{
		...
		$html[] = '<p class="rule-desc">' . rc__('JLIB_RULES_SETTINGS_DESC') . '</p>';
		$html[] = '<ul id="rules">';
		...
		foreach ($groups as $group)
		{
			...
			$html[] = '<span class="acl-action">' . rc__('JLIB_RULES_ACTION') . '</span>';
			$html[] = '</th>';

			$html[] = '<th class="settings" id="settings-th' . $group->value . '">';
			$html[] = '<span class="acl-action">' . rc__('JLIB_RULES_SELECT_SETTING') . '</span>';
			$html[] = '</th>';
			...
			if ($canCalculateSettings)
			{
				...
				$html[] = '<span class="acl-action">' . rc__('JLIB_RULES_CALCULATED_SETTING') . '</span>';
				$html[] = '</th>';
			}
			...
			foreach ($actions as $action)
			{
				...
				$html[] = '<label class="hasTip" for="' . $this->id . '_' . $action->name . '_' . $group->value . '" title="'
					. htmlspecialchars(rc__($action->title) . '::' . rc__($action->description), ENT_COMPAT, 'UTF-8') . '">';
				$html[] = rc__($action->title);
				$html[] = '</label>';
				$html[] = '</td>';
				...
				$html[] = '<select name="' . $this->name . '[' . $action->name . '][' . $group->value . ']" id="' . $this->id . '_' . $action->name
					. '_' . $group->value . '" title="'
					. rc__('JLIB_RULES_SELECT_ALLOW_DENY_GROUP', rc__($action->title), trim($group->text)) . '">';
				...
				$html[] = '<option value=""' . ($assetRule === null ? ' selected="selected"' : '') . '>'
					. rc__(empty($group->parent_id) && empty($component) ? 'JLIB_RULES_NOT_SET' : 'JLIB_RULES_INHERITED') . '</option>';
				$html[] = '<option value="1"' . ($assetRule === true ? ' selected="selected"' : '') . '>' . rc__('JLIB_RULES_ALLOWED')
					. '</option>';
				$html[] = '<option value="0"' . ($assetRule === false ? ' selected="selected"' : '') . '>' . rc__('JLIB_RULES_DENIED')
					. '</option>';
				...
				if ($canCalculateSettings)
				{
					...
					if (JAccess::checkGroup($group->value, 'core.admin') !== true)
					{
						...
						{
							$html[] = '<span class="icon-16-unset">' . rc__('JLIB_RULES_NOT_ALLOWED') . '</span>';
						}
						elseif ($inheritedRule === true)
						{
							$html[] = '<span class="icon-16-allowed">' . rc__('JLIB_RULES_ALLOWED') . '</span>';
						}
						elseif ($inheritedRule === false)
						{
							if ($assetRule === false)
							{
								$html[] = '<span class="icon-16-denied">' . rc__('JLIB_RULES_NOT_ALLOWED') . '</span>';
							}
							else
							{
								$html[] = '<span class="icon-16-denied"><span class="icon-16-locked">' . rc__('JLIB_RULES_NOT_ALLOWED_LOCKED')
									. '</span></span>';
							}
						}
					}
					...
					{
						$html[] = '<span class="icon-16-allowed"><span class="icon-16-locked">' . rc__('JLIB_RULES_ALLOWED_ADMIN')
							. '</span></span>';
					}
					else
					{
						...
						elseif ($inheritedRule === false)
						{
							...
							$html[] = '<span class="icon-16-denied"><span class="icon-16-locked">'
								. rc__('JLIB_RULES_NOT_ALLOWED_ADMIN_CONFLICT') . '</span></span>';
						}
						else
						{
							$html[] = '<span class="icon-16-allowed"><span class="icon-16-locked">' . rc__('JLIB_RULES_ALLOWED_ADMIN')
		...
		{
			$html[] = rc__('JLIB_RULES_SETTING_NOTES');
		}
		else
		{
			$html[] = rc__('JLIB_RULES_SETTING_NOTES_ITEM');
		}
		$html[] = '</div></div>';
